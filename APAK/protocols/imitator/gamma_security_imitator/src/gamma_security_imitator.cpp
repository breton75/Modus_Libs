#include "gamma_security_imitator.h"

apak::SvGammaSecurityImitator::SvGammaSecurityImitator():
...
bool apak::SvGammaSecurityImitator::configure(modus::DeviceConfig *config, modus::IOBuffer *iobuffer)
{
  try {
...
bool apak::SvGammaSecurityImitator::bindSignal(modus::SvSignal* signal, modus::SignalBinding binding)
{
  try {
...
void apak::SvGammaSecurityImitator::signalUpdated(modus::SvSignal* signal)
{
  Q_UNUSED(signal);
}

void apak::SvGammaSecurityImitator::signalChanged(modus::SvSignal* signal)
{
  Q_UNUSED(signal);
}

void apak::SvGammaSecurityImitator::start()
{
  m_timer = new QTimer;
  connect(m_timer, &QTimer::timeout, this, &SvGammaSecurityImitator::send);
  m_timer->start(m_params.send_interval);

  p_is_active = bool(p_config) && bool(p_io_buffer) && bool(m_send_signal);
}

void apak::SvGammaSecurityImitator::send()
{
...
{
  modus::SvAbstractProtocol* protocol = new apak::SvGammaSecurityImitator();
  return protocol;
}
