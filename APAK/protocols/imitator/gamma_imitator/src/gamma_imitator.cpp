#include "gamma_imitator.h"

apak::SvGammaImitator::SvGammaImitator():
...
bool apak::SvGammaImitator::configure(modus::DeviceConfig *config, modus::IOBuffer *iobuffer)
{
  try {
...
bool apak::SvGammaImitator::bindSignal(modus::SvSignal* signal, modus::SignalBinding binding)
{
  try {
...
void apak::SvGammaImitator::signalUpdated(modus::SvSignal* signal)
{
  Q_UNUSED(signal);
}

void apak::SvGammaImitator::signalChanged(modus::SvSignal* signal)
{
  Q_UNUSED(signal);
}

void apak::SvGammaImitator::start()
{
  m_timer = new QTimer;
  connect(m_timer, &QTimer::timeout, this, &SvGammaImitator::send);
  m_timer->start(m_params.send_interval);

  p_is_active = bool(p_config) && bool(p_io_buffer) && bool(m_send_signal);
}

void apak::SvGammaImitator::send()
{
...
{
  modus::SvAbstractProtocol* protocol = new apak::SvGammaImitator();
  return protocol;
}
