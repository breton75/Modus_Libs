...
#define PROC_CALL     "select %1('%2');"

#define DEFAULT_PROC_NAME "set_values"
    ...
    struct Params
    {
      ...
      static Params fromJsonObject(const QJsonObject &object) //throw (SvException)
      {
        ...
        if(object.contains(P)) {
          ...
          throw SvException(QString(MISSING_PARAM).arg(P_STORAGES).arg(P));
        ...
        /* proc name */
        P = P_PROC_NAME;
        p.proc_name = object.contains(P) ? object.value(P).toString(DEFAULT_PROC_NAME) : DEFAULT_PROC_NAME;
