...
#define PG_FUNC_SET_VALUES "select %1('%2');"
#define PG_FUNC_SET_RESERVE_VALUES "select %1('%2', '%3');"
#define PG_FUNC_SET_VALUES1 "select set_values(':s_type', ':values');"

extern "C" {

    PGDB_STORED_PROC_AGGREGATE_SHARED_EXPORT as::SvAbstractStorage* create();
...
namespace pgspa {
  ...
  class pgStoredProcAggThread;
  class pgStoredProcAggStorage;

}


class pgspa::pgStoredProcAggStorage: public as::SvAbstractStorage
{
  Q_OBJECT

public:
  pgStoredProcAggStorage(sv::SvAbstractLogger *logger = nullptr);
  ~pgStoredProcAggStorage();
  ...
  const pgspa::StorageParams* params() const { return &m_params; }

private:
  pgspa::StorageParams m_params;
...
class pgspa::pgStoredProcAggThread: public as::SvAbstractStorageThread
{
    Q_OBJECT

public:
  pgStoredProcAggThread(as::SvAbstractStorage* storage);
  ~pgStoredProcAggThread();
