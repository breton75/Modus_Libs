...
namespace oht {
  ...
  struct SignalParams_0x14
  {
    quint8 byte = 0;
    quint8 bit  = 0;

    static SignalParams_0x14 fromJson(const QString& json_string) //throw (SvException)
    {
      QJsonParseError err;
    ...
    static SignalParams_0x14 fromJsonObject(const QJsonObject &object) //throw (SvException)
    {
      SignalParams_0x14 p;
      QString P;
    ...
    QJsonObject toJsonObject() const
    {
      ...
      j.insert(P_BYTE, QJsonValue(byte).toString());
      j.insert(P_BIT,  QJsonValue(bit).toString());
  ...
  struct Signal0x14 {
    ...
    Signal0x14(modus::SvSignal* signal, SignalParams_0x14 params):
      signal(signal), params(params)
    {  }

    modus::SvSignal* signal;
    SignalParams_0x14 params;

  };

  class Type0x14 : public SvAbstractSignalCollection
  {
    Q_OBJECT
  public:
    explicit Type0x14();
