...
#define LIB_SHORT_INFO \
  "TCP сервер рассчитанный на подключение одного клиента. Взможность подключения нескольких клиентов не реализована. Интерфейсная библиотека Modus. Версия" LIB_VERSION "\n"
...
#define LIB_DESCRIPTION \
  ...
  "     1) с поддержкой одного клиента: переменной m_clientConnection назначается значение m_tcpServer->nextPendingConnection() "\
  "        и подключается к слоту read. При получении новых данных, в буфер input помещаются полученные данные и эмитируется сигнал dataReaded.\n\n"\
  "     2) с поддержкой нескольких клиентов:\n"\
  "       его сокет помещается в список подключений и подключается к слоту read. Список представляет собой список пар 'ключ:значение', "\
  ...
  "  4. При поддержке нескольких клиентов: при формировании пакета данных для клиента, протокольная библиотека должна поместить в начало буфера output идентификатор клиента (сокета). "\
