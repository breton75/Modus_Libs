...
// которые необходимы для работы tcp-сервера.
{
    ...
    emit message(QString("TCP cервер запущен на прослушивание порта %1").arg(m_params.port), lldbg, mtscc);
...
// Функция вызывается, когда серверу доступно новое соединение с клиентом.
{
    ...
    int fd = m_clientConnection->socketDescriptor();
...
// По отключению клиента -> отображаем информацию об этом в утилите "logview" и уничтожаем его TCP-сокет:
{
  ...
  if(!client)
    ...
    emit message(QString("Клиент отключился %1:%2").arg(client->peerAddress().toString()).arg(client->peerPort()), lldbg, mtfal);
...
{
  return "not relised yet";// tcp::Params::usage().toStdString().c_str();
}
