...
// в утилите "logview":
{
    // Мы оказались в состоянии отлючения от сервера:
...
// его установить:
{
    ...
    { // Если соединение с сервером не установлено, то пытаемся его установить:
        ...
        qDebug() << "TCP-клиент: Даём команду на подсоединение к серверу";
        m_client->connectToHost(m_params.server_address, m_params.port);
        return;
    }
}
...
// Запись данных в сокет:
{
  ...
  if (m_client -> state() != QAbstractSocket::ConnectedState)
  {
        // Если соединения с сервером не установлено, то некому и передавать:
        // 1. Очищаем буфер (если этого не делать, то когда соединение с TCP-сервером будет
