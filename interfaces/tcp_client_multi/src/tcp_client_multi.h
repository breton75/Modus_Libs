...
#define LIB_DESCRIPTION \
  ...
  "Пытается подключиться ко всем серверам из списка по очереди, до первого успешного. Это подключение используется, как основное для получения данных. "\
  "Для остальных серверов проверяется только возможность подключения. Подключение к серверу осуществляется с заданными праметрами " P_HOST " и " P_PORT "\n"\
